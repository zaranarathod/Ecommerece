...
Go
to [Facebook Login for Android - Quickstart](https://developers.facebook.com/docs/facebook-login/android/?locale=en)
...
- Add the following uses-permission element after the application element
  ...
  ```
- Add the following meta-data element, an activity for Facebook, and an activity and intent filter
  for Chrome Custom Tabs inside your application element
...
6. Provide the Development and Release Key Hashes for Your App
   ...
   To find info to how to generate you key hash go
   to https://developers.facebook.com/docs/facebook-login/android?locale=en_US#6--provide-the-development-and-release-key-hashes-for-your-app
...
- In the step 3 (Register and Configure Your App with Facebook) you need add
...
- In the Step 4 you need configure your `Info.plist` file inside `ios/Runner/Info.plist`
  ...
  From Xcode you can open your `Info.plist` as `Source Code` now add the next code and
...
Check if you already have CFBundleURLTypes or LSApplicationQueriesSchemes keys in your Info.plist.
...
To use any of the Facebook dialogs (e.g., Login, Share, App Invites, etc.) that can perform an app
switch to Facebook apps, your application's Info.plist also needs to include: <dict>...</dict>)
...
- `.isLogged` : check if the user has an active facebook session. The response will be `null` if the
  user is not logged.
...
- `.getUserData({String fields = "name,email,picture"})` : get the user info only if the user is
  logged.
...
🚫 *IMPORTANT:* the facebook javascript SDK is only allowed to use with `https` but you can test the
plugin in your localhost with an error message in your web console.

👉 The `isLogged` method only works in live mode using `https` and you must add your **OAuth
redirect URL**  in your *facebook developer console*.

---

Download
the [flutter_facebook_auth.js](https://raw.githubusercontent.com/darwin-morocho/flutter-facebook-auth/master/facebook_auth/example/web/flutter_facebook_auth.js)
file and put it into your `web` folder.

<img  src="https://user-images.githubusercontent.com/15864336/101827813-bf44e680-3afe-11eb-9176-4e6ec9528d0c.png" width="300" />

Now you need to define your `FACEBOOK_APP_ID` and the `flutter_facebook_auth.js` script in
your `index.html` at the top of your body tag.
